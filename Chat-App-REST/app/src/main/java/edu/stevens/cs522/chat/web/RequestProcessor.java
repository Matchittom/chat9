...
public class RequestProcessor {
    ...
    public ChatServiceResponse perform(RegisterRequest request) {
        ...
        if (response instanceof RegisterResponse) {
            ...
            String serverUri = String.valueOf(Settings.getServerUri(context));
            Settings.saveServerUri(context, Uri.parse(serverUri));
    ...
    public ChatServiceResponse perform(PostMessageRequest request) {
        ...
        if (response instanceof PostMessageResponse) {
            ...
            long seqNum = postMessageResponse.getMessageId();

            try {
                chatDatabase.requestDao().updateSeqNum(id, seqNum);
                Log.d(TAG, "Message sequence number updated: " + seqNum);
            } catch (Exception e){
                Log.e(TAG, "Error updating message sequence number", e);
            }
