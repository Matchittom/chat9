...
public class RestMethod {
    ...
    protected ServerApi createClient(Uri serverUri, ChatServiceRequest request) {
         ...
         */
        Retrofit retrofit = new Retrofit.Builder().baseUrl(serverUri.toString()).client(client).addConverterFactory(GsonConverterFactory.create(gson)).build();
    ...
    public ChatServiceResponse perform(PostMessageRequest request) {
        try {
            ...
            // TODO execute the Web service call
            Response<Void> response = server.postMessage(request.message.chatroom, request.message).execute();
            ...
            if (response.isSuccessful()) {
                ...
                long messageId = postMessageResponse.getMessageId();
                postMessageResponse.setMessageId(messageId);
